...
export class TransferenciaTercerosOtrosBancosPage {
    ...
    try {
     ...
     <soap:Body>
       <AfiliadoFavoritosGrupoGetByAfiliado xmlns="http://tempuri.org/">
         ...
         <tipoFavoritoId>111</tipoFavoritoId>
  ...
  goToTransferenciasTercerosDetalle(itemselected:any[]){
   ...
   console.log(postData);
   //Acá hacemos la llamada al servicio que nos trae el menú dinámico según el ID del user
      this.httpClient.post("http://"+this.userSession.serverIPApp+"/WsTransferenciasMovil.asmx?op=BankNameGet",postData,httpOptions )
     .subscribe(data => {
      // console.log('Data: '+data['_body']); 
      }, error => {
             //Hacemos el parse tal cual como antes:
